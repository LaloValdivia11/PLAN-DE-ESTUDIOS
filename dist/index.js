...
async function main() {
    ...
    const developerEduardo = new Developer(createEngineer.fullName, createEngineer.age, createEngineer.level, createEngineer.id);
    const teamTbaco = new Team('TBACO', [developerEduardo]);
    const createTeam = await new TeamStore(db).create(teamTbaco.name);
    const developerTwo = await new DeveloperStore(db).create(createEngineer.id, createTeam.id, createComputer.id);
}
